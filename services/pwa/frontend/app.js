...
function pushHelpText(state) {
  return ({
    ...
    unsupported_push: "Web Push is unavailable here. On iPhone/iPad, add CTO to Home Screen and open it from the installed app icon before testing.",
    ...
    subscribed: "Subscribed and provider test submitted. If no notification appears, leave this status visible and tap Report status so CTO can inspect it.",
...
async function refreshAppShell() {
  try {
    ...
    if ("caches" in window) {
      ...
      await Promise.all(keys.filter((k) => k.startsWith("cto-shell-")).map((k) => caches.delete(k)));
    }
...
async function autoReportDailyDeviceReadiness() {
  ...
  // John opens the authenticated PWA, so work pumps can verify device reality
  // without requiring him to remember the Report buttons.
...
function setupVoiceControls() {
  ...
  if (!speechOut) {
    ...
    $voiceToggle.addEventListener("click", () => {
      ...
      if (voiceEnabled) {
        const utterance = new SpeechSynthesisUtterance("Voice mode is on. New CTO replies will be read aloud.");
        window.speechSynthesis.speak(utterance);
      }
