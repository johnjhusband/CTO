...
OPENCLAW_SESSION_ID_LEGACY = os.environ.get("OPENCLAW_SESSION_ID", "pwa-john").strip() or "pwa-john"
if not OPENCLAW_SESSION_ID_BASE:
    OPENCLAW_SESSION_ID_BASE = re.sub(r"-\d{8}(?:-\d{4})?$", "", OPENCLAW_SESSION_ID_LEGACY) or "pwa-john"
...
# OpenClaw often has to inspect the CTO repo or delegate work. The PWA already
# returns 202 immediately and delivers via SSE, so give the background worker a
# long-task budget instead of killing real CTO work at the old interactive limit.
...
PUSH_SUBSCRIPTION_DIR = Path(os.environ.get("PWA_PUSH_SUBSCRIPTION_DIR", "/opt/cto/.cache/push-subscriptions"))
PWA_PUSH_ENABLED = os.environ.get("PWA_PUSH_ENABLED", "1").lower() not in {"0", "false", "no"}

PWA_JOB_PAYLOAD_DIR = Path(os.environ.get("PWA_JOB_PAYLOAD_DIR", "/opt/cto/.cache/pwa-jobs/payloads"))
...
PWA_CHAT_LOG_DIR = Path(os.environ.get("PWA_CHAT_LOG_DIR", "/opt/cto/logs/pwa-chat"))
CHAT_LOG_DATE_RE = re.compile(r"^\d{4}-\d{2}-\d{2}$")


def _clone_chat_isolation_error(*, instance_id: str, chat_db: str, cto_root: str) -> str | None:
    return clone_chat_isolation_error(instance_id=instance_id, chat_db=chat_db, cto_root=cto_root)


def _assert_clone_chat_isolation() -> None:
    error = _clone_chat_isolation_error(instance_id=CTO_INSTANCE_ID, chat_db=CHAT_DB_PATH, cto_root=CTO_ROOT)
    if error:
...
def _pwa_auth_dev_mode_allowed() -> bool:
    ...
    """
    instance = (CTO_INSTANCE_ID or "production").strip().lower()
...
def _sanitize_a2a_audit_value(value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        text = re.sub(r"(cto_pwa_session=)[^\s;]+", r"\1[REDACTED]", text, flags=re.IGNORECASE)
...
def _push_payload(*, sender: str, body: str, correlation: str | None = None) -> dict:
    ...
    label = {"openclaw": "OpenClaw", "hermes": "Hermes", "system": "CTO"}.get(sender, "CTO")
    return {
        "title": f"{label} replied",
        "body": clean or "New CTO activity",
        "tag": correlation or "cto-reply",
        "url": "/",
    ...
    try:
        ...
        try:
            subprocess.Popen(
                ...
                env={
                    **os.environ,
                    "HOME": os.environ.get("HOME", "/home/cto"),
    ...
    *,
    ...
    try:
        ...
        if e.code in {401, 403}:
            error = (
                "Hermes A2A unauthorized: cto-pwa-backend and "
                "cto-hermes-a2a-sidecar do not agree on HERMES_A2A_TOKEN, "
                "or one service has not been restarted after /opt/cto/.env changed."
            )
...
class Handler(BaseHTTPRequestHandler):
    ...
    SESSION_COOKIE_NAME = "cto_pwa_session"
    ...
    def _session_signature(cls, issued_at: int, token: str | None = None) -> str:
        msg = f"cto-pwa-session:{issued_at}".encode("utf-8")
...
<meta name="viewport" content="width=device-width, initial-scale=1" />
<title>CTO chat logs</title><link rel="stylesheet" href="/static/style.css" /></head>
<body><main class="login-card"><h1>CTO chat logs</h1>
...
<ul>{links}</ul></main></body></html>""".encode("utf-8")
    ...
    def _chat_export(self):
        ...
        parts = [f"# CTO PWA chat export — {dates[0]} to {dates[-1]}\n"]
        for day in dates:
        ...
        filename = f"cto-chat-{dates[0]}-to-{dates[-1]}.md"
        self.send_response(200)
    ...
    def do_POST(self):
        ...
        if path == "/api/push/test":
            attempted, failed = _send_push_notification(
                sender="system",
                body="CTO PWA push self-test. If you can see this, background delivery is wired.",
        ...
        if path == "/api/push/device_status":
            ...
            append(sender="system", recipient="john", kind="system_event", content=json.dumps(summary))
            return self._json(200, {"ok": True, "status": summary})
        if path == "/api/voice/device_status":
            summary = _summarize_voice_device_status(body)
            append(sender="system", recipient="john", kind="system_event", content=json.dumps(summary))
            return self._json(200, {"ok": True, "status": summary})
        return self._json(404, {"error": "not_found"})
