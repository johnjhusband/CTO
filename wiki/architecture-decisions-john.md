...
### 2. LLM Provider: ChatGPT Business (existing) via Codex OAuth — Pro as the future escape
**Settled 2026-05-11 (CTO-DECISION-008).** OpenRouter proved unreliable (ran out of credits, key limit confusion, outages). Pro $200/mo was the originally planned destination, but reality is John has a Business workspace at $30/seat — and Pro cannot be added to the same email (no Personal workspace exists). Decision: operate on John's existing Business standard seat via OpenClaw and Hermes's `openai-codex` provider with device-code OAuth (both Codex Local and device-code-auth toggles enabled at chatgpt.com/admin/settings). Business includes Codex with real 5-hour-window quotas. **Expected**: we may hit quota limits during heavy operation (John's stated expectation based on prior OpenRouter pattern). **Escape if quotas constrain**: subscribe to ChatGPT Pro on a separate email and re-point the Codex OAuth profiles to that account. OpenRouter remains the documented fallback inside Hermes/OpenClaw configs.
