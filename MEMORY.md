...
- Shared cross-hemisphere memory contract v2: OpenClaw and Hermes keep private memories separate. Durable shared knowledge lives first in the human-readable wiki/markdown layer (`/opt/cto/wiki`, decision logs, MEMORY.md, role docs, handoff docs), with Engram/SQLite treated as a searchable coordination/index/cache layer rather than the source of truth. Active repaired Engram store: `/opt/cto/.engram/engram.db` via `ENGRAM_DATA_DIR=/opt/cto/.engram`; `/opt/cto/.engram/cto.db` is a stale empty artifact from the broken Hermes MCP config unless a future decision migrates to it. Shared writes must be semantic and durable: stable decisions, John preferences, architecture facts, task handoffs, artifact pointers, research syntheses, and reusable procedures. Every material shared fact needs provenance, confidence, owner/source agent, timestamps, and supersedes/contradiction handling when replacing older claims. No transient task progress, PR numbers, stale session state, secrets, raw chain-of-thought, or private tool traces. Conflict priority: John's latest explicit instruction > OpenClaw architecture decisions > canonical decision records/role docs > newer verified tool-backed facts > agent-local memory. Intake lane: `/opt/cto/wiki/shared-memory-intake.md`; full contract: `/opt/cto/wiki/shared-memory-contract.md`. [renegotiated 2026-05-24 per Karpathy LLM Wiki pattern; CTO-DECISION-016]
