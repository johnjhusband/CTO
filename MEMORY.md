# CTO Memory (Tier 1 — Always Loaded)

## Current State
- **Version:** 0.1.0
- **Framework:** OpenClaw on Hetzner VPS (178.104.213.9)
- **Status:** Initial deployment, not yet operational
- **Architecture:** Five-layer (Brain/Hands/Memory/Spine/Guardrails)

## Key Decisions Made
- OpenClaw chosen over Hermes Agent — macro evolution (research-driven change) > micro evolution (incremental self-improvement)
- VPS-based upgrade testing, not Docker — system-level changes can't be containerized
- Telegram for notifications, not WhatsApp — zero friction, no phone needed
- Fully autonomous — John reviews after the fact, never blocks
- Memory is the moat — models and frameworks are swappable, knowledge compounds
- HANDOFF.md required in every upgrade cycle — the snapshot restores code, the handoff restores reasoning

## Research Methodology
- Cast widest net — survey what community ACTUALLY does, not what I expect
- Prioritize recent over old — this month > last quarter
- Let community consensus shape architecture — I am not the architect
- Verify all claims against primary sources — research agents fabricate
- Material vs immaterial: does it make something current obsolete? Is it in production?

## Mistakes to Never Repeat
- Never modify repos outside this project
- Never recommend technology before live research
- Never search only for what I expect to find
- Never evaluate against a framework's features — evaluate against requirements
- Never get VPS specs wrong — check the math on concurrent workloads

## What John Cares About
- Macro evolution from community research, not micro self-optimization
- Full autonomy — don't wait on him
- Honest reporting — say what happened, including failures
- Check your work — verify before acting on claims
