...
## Selected item
P0 access-control / human-interface reliability follow-up: recent service logs showed a stale mobile/PWA client repeatedly requesting the retired `/api/stream?<redacted-query-secret>` URL. The endpoint already returned 204 and did not authenticate query tokens, but the retry storm persisted.

## Action taken
- Added `Clear-Site-Data: "cache"` to the 204 response for legacy `/api/stream?<redacted-query-secret>` requests.
...
- Runtime probe of `http://127.0.0.1:8088/api/stream?<redacted-query-secret>` returned `204 No Content`, `Cache-Control: no-store`, `Clear-Site-Data: "cache"`, and an empty body.
