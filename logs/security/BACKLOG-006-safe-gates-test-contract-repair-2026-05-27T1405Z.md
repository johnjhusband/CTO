# BACKLOG-006 safe gate test-contract repair — 2026-05-27T1405Z

## Scope
The 13:58Z redaction repair cleared operational secret markers, then exposed stale PWA regression expectations from the chat-first shell change. This tick updates only the test contract to match the already-shipped settings-disclosure UI and shell v24; no PWA frontend files or services were changed.

## Verification
- `scripts/security/run-safe-security-gates.sh`
== secret artifact guard ==
Secret artifact guard passed: scanned 441 source-visible files.

== operational secret redaction check ==
Operational secret redaction check passed: scanned 293 file(s) plus chat.db; no unredacted markers found.
...
## Conclusion
Safe credential/access-control gate suite still failed; inspect this artifact before further work.
